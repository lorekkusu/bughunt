...
Mutating operations take the queue lock; ``ready_batch`` returns the entries
in dispatch order.
"""
...
class PriorityQueue:
    ...
    def ready_batch(self) -> list[Job]:
        ...
        Sorts in place and returns the backing list, avoiding a copy on the
        hot dispatch path.
        """
        with self._lock:
            self._entries.sort(key=priority.sort_key)
