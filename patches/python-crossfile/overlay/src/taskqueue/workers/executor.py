...
outcome, and schedules retries via :class:`taskqueue.core.retry.RetryPolicy`.

Leases: a claimed job carries a ``lease_deadline`` (set from
...
from taskqueue.core.retry import RetryPolicy
...
class Executor:
    ...
    def _handle_failure(self, job: Job, exc: BaseException) -> None:
        ...
        self._retry.record_failure(job)
        delay_ms = self._retry.next_delay_ms(job)
        job.transition(JobState.FAILED)
        job.transition(JobState.QUEUED)
        job.not_before = timing.now() + delay_ms / 1000.0
        self._events.append_event(job.id, "retry_scheduled", delay_ms=delay_ms)
