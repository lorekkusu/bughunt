...
In-memory, keyed by job id, guarded by an RLock. ``get_job`` returns ``None``
for unknown ids so callers can treat "missing" as a normal outcome without
exception plumbing; ``delete`` still raises :class:`JobNotFound` because
deleting something that isn't there is a caller bug.
"""
...
class Store:
    ...
    def get_job(self, job_id: str) -> Optional[Job]:
        """Return the job with *job_id*, or None for an unknown id."""
        with self._lock:
