...
Every job lifecycle change is appended as one JSON object per line:
    ...
    {"ts": <epoch seconds>, "job_id": "...", "kind": "...", ...extra fields}

``ts`` is wall-clock epoch time — events are persisted and read back across
process restarts, so a monotonic clock would be meaningless here. The file is
the *only* coupling between writers (executor, handlers) and readers (SDK
timeline, inspector): whatever schema is written here is what they parse.
"""
...
class EventLog:
    ...
    def append_event(self, job_id: str, kind: str, **extra: Any) -> dict:
        ...
        record = {"ts": time.time(), "job_id": job_id, "kind": kind}
        record.update(extra)
    ...
    def events_since(self, cutoff_ts: float) -> list[dict]:
        """Events with ``ts`` at or after the epoch-seconds *cutoff_ts*."""
