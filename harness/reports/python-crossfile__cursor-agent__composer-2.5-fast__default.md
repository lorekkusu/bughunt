# python-crossfile · cursor-agent/composer-2.5-fast · effort=`default`
...
| Speed (s) | 107.2 | 94.3 | 126.1 |
| Output tokens | 10,537 | 10006 | 10886 |
| Est. cost (USD, API-equiv) | 0.3651 | 0.3436 | 0.3879 |
...
| C3 | critical | D3 | Hook dispatch now swallows all hook exceptions; the audit plugin raises AuditReject to veto execution, so the veto is silently ignored (audit bypass) | ✅ 3/3 |
...
| M2 | medium | D1 | get_job changed to return None instead of raising, but update_state in the same file still dereferences the result — AttributeError instead of clean JobNotFound | ✅ 3/3 |
| M3 | medium | D3 | Event log ts now written in millis; the SDK timeline reader feeds it to datetime.fromtimestamp and second-based duration math (far-future dates, 1000x durations) | ⚠️ 2/3 |
