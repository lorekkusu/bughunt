# python-crossfile · claude/claude-opus-4-8 · effort=`xhigh`
...
| Bonus real bugs | 0.7 | 0 | 2 |
| Speed (s) | 291.1 | 264.8 | 319.3 |
| Output tokens | 20,293 | 18888 | 21989 |
| Est. cost (USD, API-equiv) | 1.2589 | 1.0873 | 1.4528 |
...
|----|----------|:--------:|-----|:-----:|
| C1 | critical | D2 | lease_deadline now returns epoch millis; executor compares it against time.time() seconds, so leases never expire | ⚠️ 2/3 |
...
| C3 | critical | D3 | Hook dispatch now swallows all hook exceptions; the audit plugin raises AuditReject to veto execution, so the veto is silently ignored (audit bypass) | ⚠️ 1/3 |
...
| H2 | high | D2 | cache status key renamed job:{id} -> jobs/{id}; executor still invalidates the old hardcoded key, so completed jobs serve stale status until TTL | ✅ 3/3 |
| H3 | high | D3 | New PAUSED state falls into dispatch's defensive else branch, which marks unknown-state jobs FAILED | ✅ 3/3 |
| M1 | medium | D1 | PAUSED added to JobState and is_active, but the _TRANSITIONS table in the same file has no PAUSED entries — resume raises KeyError | ✅ 3/3 |
| M2 | medium | D1 | get_job changed to return None instead of raising, but update_state in the same file still dereferences the result — AttributeError instead of clean JobNotFound | ⚠️ 1/3 |
| M3 | medium | D3 | Event log ts now written in millis; the SDK timeline reader feeds it to datetime.fromtimestamp and second-based duration math (far-future dates, 1000x durations) | ⚠️ 1/3 |
...
| L3 | low | D1 | Retry extraction left the old job.attempts += 1 in the executor while RetryPolicy.record_failure also increments — attempts counted twice, retries exhausted twice as fast | ⚠️ 2/3 |
...
- Retention never expires ms-stamped events — seen in 1/3 runs
- Inspector/DrainCoordinator iterate live queue list without lock — seen in 1/3 runs
