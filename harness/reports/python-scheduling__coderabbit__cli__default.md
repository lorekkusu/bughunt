# python-scheduling · coderabbit/cli · effort=`default`
...
| C3 | critical | is_past compares a local-naive time against datetime.utcnow() → off by the UTC offset | ⚠️ 1/3 |
| H1 | high | duration_minutes uses timedelta.seconds (ignores .days) → wrong for intervals ≥ 24h | ⚠️ 2/3 |
...
| M2 | medium | next_available treats a time touching a booking's end as busy (`<=`) → skips a valid slot | ❌ 0/3 |
| M3 | medium | monthly() adds timedelta(days=30) per step → drifts off the intended day of month | ❌ 0/3 |
...
- first_booking crashes on empty bookings list — seen in 1/3 runs
